...
public class FKMinus1F1Apriori<V> extends BaseApriori<V> {
	...
	public FKMinus1F1Apriori(List<ItemSet<V>> transactions, Double minSup,
			Double minConf) {
	...
	public List<ItemSet<V>> aprioriGen(
		...
		for (ItemSet<V> freqSet : frequentCandidatesKMinus1) {
			for (ItemSet<V> single : frequent1Itemsets) {
				if (freqSet.intersection(single).size() == 0) {
					ItemSet<V> combo = freqSet.union(single);
